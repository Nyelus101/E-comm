// //Main design before the redesigns
...
export default function ProductCard({ product }: { product: Product }) {
  ...
  return (
    <article className="group bg-white rounded-2xl overflow-hidden border border-surface-dark hover:border-amber transition-all duration-200 hover:shadow-lg flex flex-col  ">
      ...
      <Link href={`/products/${product.slug}`} className="block relative aspect-[4/3] bg-surface-alt overflow-hidden">
        {product.thumbnail_url ? (
          ...
          <div className="w-full h-full flex items-center justify-center bg-surface">
            <ShoppingCart size={36} className="text-surface-dark" />
          </div>
        )}

        {/* Badges overlay */}
        <div className="absolute top-3 left-3 flex gap-2 bg-surface-dark/50 p-1 rounded">
          {product.is_featured && (
            <Badge variant="amber">Featured</Badge>
          )}
          {discount && (
      ...
      <div className="p-4 flex flex-col flex-1 gap-3">
        ...
        {/* Brand + name */}
        <div>
          <p className="text-[11px] font-body text-ink-faint uppercase tracking-widest mb-1">
            {product.brand}
          </p>
          <Link href={`/products/${product.slug}`}>
            <h3 className="font-display font-500 text-sm text-ink leading-snug hover:text-amber-dark transition-colors line-clamp-2">
              {product.name}
            </h3>
          </Link>
        </div>
        ...
        {/* Divider */}
        <hr className="border-t border-surface-dark" />
        ...
        <div className="flex items-end justify-between gap-3">
          ...
          <button
            onClick={() => addItem(product.id)}
            disabled={!product.is_available || isLoading}
            className="flex items-center gap-2 bg-ink text-white text-xs font-display font-600 px-4 py-2.5 rounded-xl hover:bg-amber-dark hover:text-ink transition-all duration-150 disabled:opacity-35 disabled:cursor-not-allowed active:scale-[0.97] whitespace-nowrap"
          >
